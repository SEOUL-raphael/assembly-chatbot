...
import { useSendChatMessage, type ChatMessage, type ToolCall } from "@workspace/api-client-react";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { Send, Building2, User, Bot, AlertCircle, ChevronDown, ChevronUp, Loader2, Menu, X } from "lucide-react";
      ...
      <header className="bg-primary text-primary-foreground py-3 px-4 shadow-sm z-10 flex items-center">
        <div className="max-w-4xl w-full mx-auto flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <p className="text-primary-foreground/75 text-[10px] sm:text-xs mt-0.5 truncate">
              국회 공공데이터 기반 지능형 질의응답 서비스
            </p>
          </div>
        </div>
      </header>
            ...
            <div className="space-y-4 sm:space-y-6 max-w-3xl mx-auto pb-4">
              ...
              {chatMutation.isError && (
                <Alert variant="destructive" className="max-w-3xl mx-auto mt-4">
                  ...
                  <AlertDescription>
                    {(chatMutation.error as Error)?.message || "서버와 통신하는 중 문제가 발생했습니다."}
                  </AlertDescription>
                </Alert>
              )}
            </div>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <div className="relative rounded-2xl border border-gray-200 bg-white shadow-sm focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                <Textarea
                  ...
                  className="min-h-[52px] max-h-[160px] border-0 focus-visible:ring-0 resize-none py-3.5 px-4 pr-14 bg-transparent text-sm"
                  rows={1}
                />
              ...
              <div className="text-center mt-2 space-y-0.5">
                <p className="text-[11px] text-gray-400">
                  AI가 제공하는 정보는 국회 공공 API를 기반으로 하며, 시점에 따라 실제와 차이가 있을 수 있습니다.
                </p>
                <p className="text-[11px] text-gray-300">
                  LLM: MiniMax M2.7 &nbsp;·&nbsp; 국회 데이터:{" "}
                  <a
                    href="https://github.com/hollobit/assembly-api-mcp"
                    target="_blank"
                  ...
                  {" "}&nbsp;·&nbsp; 법령:{" "}
                  <a
                    href="https://github.com/chrisryugj/korean-law-mcp"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="underline underline-offset-2 hover:text-gray-400 transition-colors"
                  >
  ...
  return (
    <div className={`flex items-start gap-2.5 sm:gap-4 ${isUser ? "flex-row-reverse" : ""}`}>
      <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center shrink-0 ${
        isUser ? "bg-gray-800" : "bg-primary/10"
      }`}>
        {isUser
      ...
      <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} min-w-0 max-w-[88%] sm:max-w-[85%]`}>
        <div className={`rounded-2xl px-4 py-3 shadow-sm text-sm sm:text-[15px] leading-relaxed ${
          isUser
      ...
      <CollapsibleContent>
        <div className="px-3 py-2.5 sm:px-4 sm:py-3 border-t border-gray-100 bg-white/50">
          <div className="mb-2">
            ...
            <pre className="text-[10px] sm:text-xs bg-gray-100 p-2 rounded-md overflow-x-auto text-gray-700 max-h-[120px]">
              {toolCall.arguments}
            </pre>
          </div>
          {formattedResult && (
            <div>
              ...
              <pre className="text-[10px] sm:text-[11px] bg-gray-900 text-gray-100 p-2.5 sm:p-3 rounded-md overflow-x-auto max-h-[200px] sm:max-h-[300px]">
                {formattedResult}
              </pre>
            </div>
          )}
